<section class="panel">
	<header class="panel-heading">
		<?php if(get_permission('laju_prima', 'is_add')){ ?>
		<div class="panel-btn">
		...
		<?php } ?>
		<h4 class="panel-title"><i class="fas fa-list-ul"></i> Data P.O Laju Prima</h4>
	</header>
	<div class="panel-body">
		<div class="export_title">Laporan Data Mobil Laju Prima</div>
		<table class="table table-bordered table-hover table-condensed table_default" cellspacing="0" width="100%">
			...
			<tbody>
				...
				<tr>
					...
					<td>
                        <?php if (get_permission('laju_prima', 'is_edit')): ?>
                        <a href="javascript:void(0);" class="btn btn-circle btn-default icon" data-toggle="tooltip"
                            data-original-title="Edit" onclick="EditData('<?=$row['id']?>','lajuprima');"><i class="fas fa-pen-nib"></i></a>
						<?php endif; if (get_permission('laju_prima', 'is_delete')): ?>
...
<div id="add_modal" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
	<section class="panel">
        ...
        <input type="hidden" name="po" value="lajuprima">
		<div class="panel-body">
