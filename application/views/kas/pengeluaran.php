<section class="panel">
	<header class="panel-heading">
		<?php if(get_permission('kas_keluar', 'is_add')){ ?>
		<div class="panel-btn">
			<a href="javascript:void(0);" class="add_barang btn btn-default btn-circle">
				<i class="fas fa-plus-circle"></i> Tambah Kas keluar
			</a>
		</div>
		<?php } ?>
	...
	<div class="panel-body">
		...
		<table class="table table-bordered table-hover table-condensed table_default" cellspacing="0" width="100%">
			...
			<tbody>
                <?php $count = 1; if (!empty($pengeluaran)){ foreach ($pengeluaran as $row): ?>
				<tr>
					...
					<td>
						<?php if (get_permission('kas_keluar', 'is_edit')): ?>
                        <a href="javascript:void(0);" class="btn btn-circle btn-default icon" data-toggle="tooltip"
                            data-original-title="Edit" onclick="KasKeluar('<?=$row['id']?>');"><i class="fas fa-pen-nib"></i></a>
						<?php endif; if (get_permission('kas_keluar', 'is_delete')): ?>
						<?=btn_delete('kas/aksi_pengeluaran/hapus/' . $row['id']); ?>
						<?php endif; ?>
					</td>
				</tr>
				<?php endforeach; }?>
...
<div id="add_modal" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
	<section class="panel">
		<?php echo form_open_multipart(base_url('kas/aksi_pengeluaran/simpan'), array('class' => 'form-horizontal')); ?>
		<div class="panel-heading">
			<h4 class="panel-title">
				<i class="fas fa-bars"></i> Tambah Pengeluaran
			</h4>
		</div>
